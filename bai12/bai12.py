# Định nghĩa menu hiển thị gợi ý người dùng nhập tuỳ chon (option)
USER_MENU = """Nhập
a - Thêm một bộ phim mới
l - Hiển thị danh sách phim
s - Tìm kiếm các bộ phim theo tên 
x - Xoá phim theo tên
u - Cập nhật thông tin của tên phim
q - Thoát
Lựa chọn của bạn: """

# Định nghĩa cấu trúc dữ liệu lưu trữ của các bộ phim
...
def remove_movie():  
    if movies:
        ...
        for idx, movie in enumerate(movies):
            if movie["name"] == movie_name:
                del movies[idx]
                print("Đã xoá bộ phim thành công!")
                break
            else:
                print("Không tìm thấy bộ phim có tên là",movie_name)
    else:
...
while user_choise != "q":
    # Kiểm tra tuỳ chọn có nằm trong operations dict hay không
    # Nếu có thì gọi hàm không thì in ra "Lựa chọn không hợp lệ, yêu cầu nhập lại!"
    if(user_choise in operations):
        # Lấy ra giá trị của key: user_choice
        operation = operations[user_choise]
        operation()
    else:
